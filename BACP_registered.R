...
result_saved <- read.csv("https://raw.githubusercontent.com/MHWauben/UK_MH_services/population_counts/bacp_postcodes.csv")
...
postalDistrict$geometry <- sf::st_transform(postalDistrict$geometry, crs = "+proj=longlat +datum=WGS84")

# Merge and visualise data
postcode_poly <- st_as_sf(merge(postcode_all, postalDistrict))

# Number of members per capita (cube root & scaled)
plot(postcode_poly["memb_percap_cubrt_sc"], lty = 0)
# Number of members per capita (rank & scaled)
plot(postcode_poly["memb_percap_ra_sc"], lty = 0)

# Do the same with leaflet for interactivity
pal <- colorNumeric(
  palette = "YlGnBu",
  domain = postcode_poly$memb_percap_cubrt_sc)

leaflet(postcode_poly) %>%
  addProviderTiles(providers$Stamen.TonerLite, group = "Toner Lite") %>%
  addPolygons(label = ~memb_percap,
              color = ~pal(memb_percap_cubrt_sc), weight = 1, smoothFactor = 0.5,
